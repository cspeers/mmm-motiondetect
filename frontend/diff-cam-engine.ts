...
declare type IUserMediaConstraints = IMediaConstraints<
  boolean | AudioCapabilities
>;
...
interface ICameraDifferenceEngine extends ICameraDifferenceOptions {
   ...
   */
  initialize(
    options: ICameraDifferenceOptions
  ): void | Promise<ICameraDifferenceEngine>;
...
class CameraDifferenceEngineClass implements ICameraDifferenceEngine {
  ...
  private calculateMotionBox(
    currentMotionBox: MotionBox,
  ...
  private processDifference(image: ImageData): IDifference {
    ...
    for (let i = 0; i < rgba.length; i += 4) {
      ...
      const normalized = Math.min(255, pixelDiff * (255 / this.pixelThreshold));
      rgba[i] = 0;
      rgba[i + 1] = normalized;
      rgba[i + 2] = 0;
      if (pixelDiff >= this.pixelThreshold) {
        ...
        if (this.includeMotionBox || this.includeMotionPixels) {
          ...
          if (this.includeMotionBox) {
            motionBox = this.calculateMotionBox(
              motionBox,
              coords,
              coords.x,
              coords.y
  ...
  private capture(): void {
    ...
    const captureImageData = this.captureContext.getImageData(
      0,
      0,
    ...
    this.differenceContext.drawImage(
      this.captureCanvas,
      0,
      0,
      this.differenceSize.width,
      this.differenceSize.height
    );
    const diffImageData = this.differenceContext.getImageData(
      0,
      0,
      this.differenceSize.width,
      this.differenceSize.height
    ...
    if (this.readyToDifference) {
      const difference = this.processDifference(diffImageData);
      ...
      const result: IDifferenceResult = {
  ...
  async initialize(
    options: ICameraDifferenceOptions
  ): Promise<ICameraDifferenceEngine> {
    try {
      ...
      this.onStreamReadyCallback =
        options.onStreamReadyCallback || function (i) {};
      this.onImageCaptureCallback =
        options.onImageCaptureCallback || function (i) {};
      this.onStreamStopCallback =
        options.onStreamStopCallback || function (i) {};
      this.onDifferenceCallback =
        options.onDifferenceCallback || function (r) {};
      ...
      this.log.info(
        `Intializing User Media Stream (${this.captureSize.width}x${this.captureSize.height})...`
      );
  ...
  async start(): Promise<ICameraDifferenceEngine> {
    try {
      this.log.info(
        `Starting UserMedia Capture to Stream with interval... ${this.captureInterval}`
      );
      if (!this.stream) {
